"""
Build MLB daily-view artifacts from real MLB odds artifacts.

This creates the first MLB model layer inside the NBA dashboard contract. It
...
from configs.leagues.league_mlb import (
...
MODEL_VERSION = "MLB_MULTI_MODEL_V1"
CALIBRATION_VERSION = "MLB_RUNS_MODEL_SEED_V1"
MARKET_CONSENSUS_MODEL_NAME = "MLB_MarketConsensus_v1"
SPREAD_VALUE_MODEL_NAME = "MLB_SpreadValue_v1"
TOTAL_VALUE_MODEL_NAME = "MLB_TotalValue_v1"
MARKET_VALUE_MODEL_NAME = "MLB_MarketValueBlend_v1"
RUNS_BASELINE_MODEL_NAME = "MLB_RunsBaseline_v1"
LAST5_RUNS_MODEL_NAME = "MLB_Last5Runs_v1"
MARKET_PRESSURE_MODEL_NAME = "MLB_MarketPressure_v1"
MODEL_NAME = "MLB_MarketBlend_v1"
RESULT_HISTORY_DIR = PROJECT_ROOT / "data" / "mlb" / "raw"
...
def _load_result_history() -> list[dict]:
    ...
    for path in sorted(RESULT_HISTORY_DIR.glob("mlb_historical_results_*.json")):
        try:
...
def _team_run_stats(team: str, history: list[dict]) -> dict:
    key = _norm_team(team)
    games = []
    for row in history:
        ...
        if key == home:
            games.append(
                {
                    "date": _safe_text(row.get("requested_date")),
                    "runs_for": home_score,
    ...
    return {
        "games": len(games),
        "avg_runs_for": _median([g["runs_for"] for g in games]),
        "avg_runs_against": _median([g["runs_against"] for g in games]),
        "last5_games": len(last5),
        "last5_runs_for": _median([g["runs_for"] for g in last5]),
        "last5_runs_against": _median([g["runs_against"] for g in last5]),
    }
...
def _score_model(
    *,
    ...
    return {
        ...
        "confidence_reason": (
            f"{model_name} compares run projection to current run line/total; "
...
def _build_run_models(home: str, away: str, game_date: str, consensus: dict) -> dict[str, dict]:
    ...
    home_stats = _team_run_stats(home, history)
    away_stats = _team_run_stats(away, history)

    if all(home_stats.get(k) is not None for k in ("avg_runs_for", "avg_runs_against")) and all(
        away_stats.get(k) is not None for k in ("avg_runs_for", "avg_runs_against")
    ):
        baseline_home = (home_stats["avg_runs_for"] + away_stats["avg_runs_against"]) / 2.0
        baseline_away = (away_stats["avg_runs_for"] + home_stats["avg_runs_against"]) / 2.0
        baseline = _score_model(
            model_name=RUNS_BASELINE_MODEL_NAME,
            ...
            context_flags={
                ...
                "home_avg_runs_for": home_stats["avg_runs_for"],
                "home_avg_runs_against": home_stats["avg_runs_against"],
                "away_avg_runs_for": away_stats["avg_runs_for"],
                "away_avg_runs_against": away_stats["avg_runs_against"],
            },
        )
    else:
        baseline = _null_model(
            RUNS_BASELINE_MODEL_NAME,
            {"source": "espn_completed_results_before_slate", "warning": "Insufficient season run history."},
        )

    if all(home_stats.get(k) is not None for k in ("last5_runs_for", "last5_runs_against")) and all(
        away_stats.get(k) is not None for k in ("last5_runs_for", "last5_runs_against")
    ):
        last5_home = (home_stats["last5_runs_for"] + away_stats["last5_runs_against"]) / 2.0
        last5_away = (away_stats["last5_runs_for"] + home_stats["last5_runs_against"]) / 2.0
        last5 = _score_model(
            model_name=LAST5_RUNS_MODEL_NAME,
            ...
            context_flags={
                ...
                "home_last5_runs_for": home_stats["last5_runs_for"],
                "home_last5_runs_against": home_stats["last5_runs_against"],
                "away_last5_runs_for": away_stats["last5_runs_for"],
                "away_last5_runs_against": away_stats["last5_runs_against"],
            },
        )
    else:
        last5 = _null_model(
            LAST5_RUNS_MODEL_NAME,
            {"source": "espn_completed_results_before_slate", "warning": "Insufficient last-5 run history."},
        )
    ...
    if base_total is not None and base_margin is not None and market_total is not None:
        ...
        pressure = _score_model(
            ...
            away=away,
            context_flags={
                "source": "runs_baseline_regressed_to_market_total",
        ...
        pressure = _null_model(
            ...
            {"source": "runs_baseline_regressed_to_market_total", "warning": "Missing baseline or market total."},
        )
    ...
    if base_total is not None and base_margin is not None and market_home is not None and market_away is not None:
        ...
        blend = _score_model(
            ...
            away=away,
            context_flags={
                "source": "runs_baseline_market_blend",
        ...
        blend = _null_model(
            MODEL_NAME,
            {"source": "runs_baseline_market_blend", "warning": "Missing baseline or market-implied scores."},
        )

    return {
        RUNS_BASELINE_MODEL_NAME: baseline,
        LAST5_RUNS_MODEL_NAME: last5,
...
def _load_flattened_rows() -> list[dict]:
    if not BETLINES_FLATTENED_CSV_PATH.exists():
        raise FileNotFoundError(f"Missing MLB flattened odds CSV: {BETLINES_FLATTENED_CSV_PATH}")
    with BETLINES_FLATTENED_CSV_PATH.open("r", encoding="utf-8-sig", newline="") as f:
        rows = list(csv.DictReader(f))
    if not rows:
        raise ValueError(f"MLB flattened odds CSV is empty: {BETLINES_FLATTENED_CSV_PATH}")
    return rows
...
def _confidence_from_edges(spread_edge: float, total_edge: float, book_count: int) -> tuple[str, str]:
    ...
    if book_count < MIN_BOOKS_FOR_VALUE_PICK:
        return "IGNORE", "Insufficient bookmaker coverage for MLB market-value signal."
    if best_edge >= 2.0:
    ...
    return "IGNORE", "No actionable MLB line-value gap versus consensus."
...
def _model_result(consensus: dict) -> dict:
    ...
    if not spread_pick and not total_pick:
        confidence_tier = "IGNORE"
        confidence_reason = "No MLB line-value pick cleared the configured thresholds."
    if confidence_tier == "IGNORE":
        spread_pick = ""
        total_pick = ""
    actionability = "ACTION" if confidence_tier in ("STRONG", "WATCH") and (spread_pick or total_pick) else "NONE"
    return {
        ...
        "context_flags": {
            ...
            "warning": "MLB market-value model only; no historical ROI/backtest attached.",
        },
    }
...
def _structured_game(game_rows: list[dict]) -> dict:
    ...
    run_models = _build_run_models(home, away, game_date, consensus)
    model = run_models.get(MODEL_NAME) or market_value_model
    ...
    return {
        ...
        "context_flags": {
            ...
            "away_3pt_pct": None,
            "three_pt_diff": None,
            "mlb_model_status": "multi_model_runs_seed",
            "mlb_spread_value_book": value_flags.get("spread_value_book", ""),
            "mlb_spread_value_line": value_flags.get("spread_value_line"),
            "mlb_total_value_book": value_flags.get("total_value_book", ""),
            "mlb_total_value_line": value_flags.get("total_value_line"),
        },
        "execution_overlay": {},
        "pocket_score": None,
        "pocket": None,
        "calibration_tags": {
            ...
            "model_regime_normal": "MLB_MARKET_VALUE_NO_BACKTEST",
        },
        "temporal_integrity": {
...
def build_mlb_daily_view(date: str | None = None) -> dict:
    ensure_mlb_dirs()
    ...
    if not games:
        raise ValueError(f"No MLB games available for date={date or 'ALL'} from {BETLINES_FLATTENED_CSV_PATH}")
    ...
    for game_date in sorted({g["identity"]["game_date_local"] for g in games}):
        ...
        payload = {
            ...
            "is_roi_output": False,
            "model_warning": (
                "MLB Daily View uses a cross-book market-value model. Picks mean available line value "
                "versus consensus, not historical ROI or win-probability confidence. No MLB ROI, pocket "
        ...
        snapshot_path = daily_dir / "snapshots" / f"daily_view_mlb_{game_date}_v1_{timestamp}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build MLB NBA-contract Daily View from flattened odds.")
    parser.add_argument("--date", help="Optional slate date YYYY-MM-DD. Default builds all dates in flattened odds.")
    args = parser.parse_args()
    result = build_mlb_daily_view(args.date)
    print(
        f"[mlb_daily_view] games={result['game_count']} "
