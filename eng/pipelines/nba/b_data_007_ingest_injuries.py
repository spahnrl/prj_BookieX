...
OUT_DIR = DERIVED_DIR
HISTORY_JSON = DERIVED_DIR / "nba_injuries_history.json"
HISTORY_CSV = DERIVED_DIR / "nba_injuries_history.csv"
...
def run():
    ...
    if not new_rows:
        print("[INFO] No injuries returned from ESPN.")
        return
    ...
    if snapshot_date in existing_dates:
        print(f"[INFO] Snapshot for {snapshot_date} already recorded. Skipping append.")
        return
    ...
    print(f"[OK] Appended snapshot for {snapshot_date}")
