"""
Build a WNBA prediction ledger seed from existing WNBA daily model artifacts.

Scope:
- Fetch real settled WNBA results from ESPN scoreboard for available daily dates.
- Join results to existing WNBA daily model predictions.
...
DERIVED_DIR = PROJECT_ROOT / "data" / "wnba" / "derived"
RAW_DIR = PROJECT_ROOT / "data" / "wnba" / "raw"
BACKTEST_DIR = PROJECT_ROOT / "data" / "wnba" / "backtests"
INTERIM_DIR = PROJECT_ROOT / "data" / "wnba" / "interim"
...
def _requests_get(url: str, **kwargs):
    ...
    try:
        ...
        print(f"SSL verification failed for {url}; retrying WNBA results fetch with verify=False: {exc}")
    ...
    for path in _daily_paths():
        ...
        for game in games:
            ...
            if not any(model for _, model in candidate_models):
                ...
                candidate_models = [(_safe_text(model.get("model_name")) or "WNBA_MarketBlend_v1", model)]
...
def _fetch_results(date_list: list[str]) -> list[dict]:
    ...
    for date_str in date_list:
        ...
        rows.extend(daily)
        print(f"[wnba_ledger] ESPN {date_str}: events={len(payload.get('events') or [])} normalized={len(daily)}")
    deduped = {}
    for row in rows:
...
def _ledger_rows(predictions: list[dict], results: list[dict], flat_rows: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for pred in predictions:
        ...
        selection_authority = _safe_text(pred.get("selection_authority")) or "WNBA_MarketBlend_v1"
...
def build_ledger(start_date: str | None = None, end_date: str | None = None) -> dict:
    ...
    if not predictions:
        raise SystemExit("No WNBA predictions with picks found in daily model artifacts.")
    ...
    raw_json = RAW_DIR / f"wnba_historical_results_{start_label}_{end_label}.json"
    raw_csv = RAW_DIR / f"wnba_historical_results_{start_label}_{end_label}.csv"
    ledger_json = BACKTEST_DIR / "wnba_prediction_ledger_seed.json"
    ledger_csv = BACKTEST_DIR / "wnba_prediction_ledger_seed.csv"
    stamped_ledger_json = BACKTEST_DIR / f"wnba_prediction_ledger_seed_{stamp}.json"
    audit_csv = INTERIM_DIR / "wnba_prediction_ledger_join_audit.csv"

    _write_json(raw_json, {"captured_at_utc": datetime.now(UTC).isoformat().replace("+00:00", "Z"), "source": "espn_public_scoreboard_wnba", "dates": date_list, "results": results})
    _write_csv(raw_csv, results)
    payload = {
        "league": "wnba",
        "artifact_type": "prediction_ledger_seed",
        "generated_at_utc": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
        "source": "wnba_daily_model_plus_espn_results",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build WNBA historical prediction ledger seed.")
